...
def generate_keys(key):
    ...
    left = key_p10[:5]
    right = key_p10[5:]
    # 3. Shift de 1 para K1
    left = left_shift(left, 1)
    right = left_shift(right, 1)
    k1 = permute(left + right, P8)
    # 4. Shift de 2 para K2
...
def encrypt(plaintext: str, key: str) -> str:
    if len(plaintext) != 8 or len(key) != 10:
        st.error("Texto plano deve ter 8 bits e a chave deve ter 10 bits!")
        return ""

    st.info("Iniciando a cifragem do S-DES...")
    ...
    # 3. Primeira função Fk com K1
    first_fk = fk(ip_bits, k1)

    # 4. Switch SW
    switched = first_fk[4:] + first_fk[:4]

    # 5. Segunda função Fk com K2
